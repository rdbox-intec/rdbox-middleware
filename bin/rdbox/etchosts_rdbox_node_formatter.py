...
class EtchostsRdboxNodeFormatter(RdboxNodeFormatter):
    def output_report(self, rdbox_node_list):
        ...
        for i, rdbox_node in enumerate(raw_list):
            if i == last_index:
                output_str += "%s %s %s" % (rdbox_node.get_ip(), rdbox_node.get_hostname(), self._get_fqdn(rdbox_node.get_hostname()))
            else:
                output_str += "%s %s %s\n" % (rdbox_node.get_ip(), rdbox_node.get_hostname(), self._get_fqdn(rdbox_node.get_hostname()))
