...
class GetCommand(object):
    ...
    def execute(self):
        intermediate_data = self.intermediate_data(self.collect_input_data_algorithm, self.convert_intermediate_data_algorithm)
        intermediate_data = intermediate_data.call()
        final_data = self.final_data(intermediate_data)
        report_data = self.report_data(final_data, self.convert_report_data_algorithm)
...
class GetCommandNode(GetCommandBuilder):
    def setting(self):
        ...
        ###
        self.set_convert_intermediate_data_algorithm(K8sResponseHelperV1NodeList())
...
class GetCommandK8sExternalSvc(GetCommandBuilder):
    def setting(self):
        ...
        ###
        k8s_client_list = K8sClientList()
        k8s_client_list.add(K8sClientCoreV1Api("list_service_for_all_namespaces"))
        k8s_client_list.add(K8sClientExtensionsV1beta1Api("list_ingress_for_all_namespaces"))
        self.set_collect_input_data_algorithm(k8s_client_list)
        ###
        self.set_convert_intermediate_data_algorithm(K8sResponseHelperV1beta1IngressList())
